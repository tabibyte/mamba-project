...
*  Karesel $\mathcal{O}(N^2)$ Transformer maliyetine karşı, doğrusal $\mathcal{O}(N)$ zaman karmaşıklığı ile sabit RAM/VRAM tüketimi.
...
2. **Georgia 12-Lead ECG Challenge** (10.344 kayıt, 500Hz $\rightarrow$ 100Hz'e alt örneklendi)
3. **Chapman-Shaoxing** (10.247 kayıt, 100Hz, Dinamik Kırpma/Sıfır Dolgusu)
4. **China Physiological (CPSC 2018)** (6.877 kayıt, 500Hz $\rightarrow$ 100Hz)
...
* **Stem:** 1D Conv (Kernel 15) $\rightarrow$ GELU $\rightarrow$ 1D Conv (Kernel 7) [Sinyali 4 kat alt örnekler]
* **Backbone:** 6x `BiMambaBlock` ($D_{model}=256$, $D_{state}=32$). Sinyali eşzamanlı olarak ileri ve geri yönde tarayarak bağlam kurar.
* **Head:** RMSNorm $\rightarrow$ **Global Max Pooling** $\rightarrow$ MLP $\rightarrow$ 5 Sınıflı Logit Çıkışı

### 2. Advanced ECG Transformer (Karşılaştırma / Baseline)
* **Stem:** Mamba ile tamamen aynı evrişimli kök mimarisi.
* **Backbone:** Positional Encoding $\rightarrow$ 6x Transformer Encoder Katmanı (8 Head, Pre-LN kararlılık ayarı).
* **Head:** LayerNorm $\rightarrow$ **Global Average Pooling** $\rightarrow$ MLP $\rightarrow$ 5 Sınıflı Logit Çıkışı
...
| **Zaman Karmaşıklığı**| $\mathcal{O}(N)$ | $\mathcal{O}(N^2)$ |
